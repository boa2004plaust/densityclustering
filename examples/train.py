...
from dc.utils.serialization import load_checkpoint, save_checkpoint
...
def main_worker(args):
    ...
    print('--------------------------------------------')
    evaluator.evaluate_classification(-1, test_loader, dataset.test, dataset.train)
    print('--------------------------------------------')
    ...
    for epoch in range(args.epochs):
        with torch.no_grad():
            ...
            features = torch.cat([features[f].unsqueeze(0) for f, _ in sorted(dataset.train)], 0)
        ...
        print('--------------------------------------------')
        evaluator.prediction_for_clustering(epoch, dataset.train, pseudo_labels, num_cluster)
        evaluator.evaluate_clustering(epoch, dataset.train, pseudo_labels)
        ...
        for i, ((fname, _), label) in enumerate(zip(sorted(dataset.train), pseudo_labels)):
            if label != -1:
        ...
        if (epoch + 1) % args.eval_step == 0 or (epoch == args.epochs - 1):
            print('--------------------------------------------')
            Top1, _, _ = evaluator.evaluate_classification(epoch, test_loader, dataset.test, dataset.train)
            print('--------------------------------------------')
            ...
            print('\n * Finished epoch {:3d}  model Top1: {:5.1%}  best: {:5.1%}{}\n'.
