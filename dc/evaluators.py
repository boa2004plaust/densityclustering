...
def pairwise_distance(features, test=None, train=None):
    ...
    x = torch.cat([features[f].unsqueeze(0) for f, _ in test], 0)
    y = torch.cat([features[f].unsqueeze(0) for f, _ in train], 0)
...
class Evaluator(object):
    ...
    def feature_extraction_for_evluation(self, data_loader, test, train):
        ...
        distmat, test_features, train_features = pairwise_distance(features, test, train)
        test_ids = np.array([pid for _, pid in test])
        train_ids = np.array([pid for _, pid in train])
    ...
    def evaluate_classification(self, epoch, data_loader, test, train, K=5, sigma=0.07):
        distmat, test_features, train_features, test_ids, train_ids = self.feature_extraction_for_evluation(data_loader, test, train)
        ...
        pred_one_hot.scatter_(1, test_pred_topk.view(-1, 1), 1)
        ...
        top5 = correct.narrow(1, 0, 5).sum().item() / N
        ...
        res = {'ACC': acc, 'NMI': nmi, 'ARI': ari, 'Top-1': top1, 'Top-5': top5}

        print("#Classification Epoch %d, ACC: %.1f%%, NMI: %.1f%%, ARI: %.1f%%, Top-1: %.1f%%, Top-5: %.1f%%." % (
            epoch, res['ACC'] * 100, res['NMI'] * 100, res['ARI'] * 100,
            res['Top-1'] * 100, res['Top-5'] * 100))

        return res['Top-1'], train_features, train_ids

    def evaluate_clustering(self, epoch, train, predictions):
        targets = np.array([pid for _, pid in train])
    ...
    def prediction_for_clustering(self, epoch, train, pseudo_labels, num_cluster):
        clustered_samples = 0
        ...
        for x in range(num_cluster):
            ...
            count = 0
            data_idx = []
            for i, ((fname, id), y) in enumerate(zip(sorted(train), pseudo_labels)):
                if (y == x):
                    print(id, end=',')
                    data_idx.append(i)
                    count = count + 1
